    ...
    <div className="h-full">
       {taskList.map((task, index) => (
        <div key={index} data-id={index} className="p-4 border-b border-gray-300 flex items-center h-18">
            ...
            <button className="ml-auto mr-7 h-6 w-6" onClick={deleteTask}><img src="./src/assets/trash-solid-full.svg" className="h-full cursor-pointer pointer-events-none" /></button>
        </div>
       ))}
    </div>);
}
